...
app.patch("/api/admin/settings", authenticate, requireRoles("admin", "editor"), async (req, res) => {
  ...
  try {
    ...
    // business_name y logo_url NO se escriben desde aquí: PATCH /api/admin/organization
    // ya los sincroniza en organization_settings cada vez que cambian (ver esa ruta).
    // Antes esta consulta los reescribía también, con el `form` operativo del frontend
    // (pestañas Contacto/Legal/Portada) — que nunca se entera de un cambio de logo hecho
    // en la pestaña Marca. Guardar cualquier otra pestaña después borraba el logo en
    // silencio, sin error visible. $1/$2 quedan sin usar a propósito (self-assignment).
