...
export default function PhoneField({ label, value, onChange, defaultCountry = DEFAULT_PHONE_COUNTRY, hint, required = false }) {
  ...
  return (
    <div className="phone-field">
      <span className="phone-field-label">{label}{required ? " *" : ""}</span>
      <div className="phone-field-control">
        <label className="phone-country-select">
          ...
          <select value={country} onChange={handleCountryChange} aria-label={`País de ${label.toLowerCase()}`}>
            {PHONE_COUNTRIES.map((item) => <option value={item.code} key={item.code}>{item.flag} {item.name} ({item.dialCode})</option>)}
          </select>
          <span aria-hidden="true">{selectedCountry?.flag || "🌐"} {selectedCountry?.dialCode || "+"}</span>
        </label>
        <input type="tel" inputMode="tel" autoComplete="tel" value={inputValue} onChange={handleInput} onBlur={handleBlur} placeholder={selectedCountry?.code === "DO" ? "829 944 0111" : "Número de teléfono"} required={required} aria-label={label} />
      </div>
      {internationalPreview ? <small className="phone-field-preview">Se guardará como {internationalPreview}</small> : <small className="phone-field-hint">{hint || "Selecciona el país e introduce el número local."}</small>}
    </div>
  );
}
