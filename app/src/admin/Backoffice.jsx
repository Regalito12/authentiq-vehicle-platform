...
function TrustContentFields({ form, onChange }) {
  ...
  return <div className="settings-section trust-content-settings"><span className="eyebrow">CONFIANZA Y CONTENIDO</span><p className="settings-section-note">Responde las dudas que más escuchas y publica opiniones autorizadas. Si no agregas testimonios, esa sección no aparece en tu showroom.</p><div className="trust-editor-block"><div className="settings-section-heading"><div><h3>Preguntas frecuentes</h3><p>Hasta 12 preguntas claras para ayudar al comprador a decidir.</p></div><button type="button" className="secondary-action" disabled={faqItems.length >= 12} onClick={() => onChange("faqItems", [...faqItems, { question: "", answer: "" }])}>Agregar pregunta</button></div>{faqItems.length ? <div className="trust-editor-list">{faqItems.map((item, index) => <article className="trust-editor-item" key={`faq-${index}`}><div className="trust-editor-item-head"><strong>Pregunta {index + 1}</strong><button type="button" className="text-button danger-text" onClick={() => removeItem("faqItems", index)}>Quitar</button></div><label>Pregunta<input value={item.question || ""} maxLength="180" onChange={(event) => updateItem("faqItems", index, "question", event.target.value)} placeholder="¿Puedo agendar una visita?" /></label><label>Respuesta<textarea value={item.answer || ""} maxLength="1200" rows="3" onChange={(event) => updateItem("faqItems", index, "answer", event.target.value)} placeholder="Explica el siguiente paso de forma sencilla." /></label></article>)}</div> : <p className="trust-editor-empty">No hay preguntas personalizadas. El showroom usará las preguntas base.</p>}</div><div className="trust-editor-block"><div className="settings-section-heading"><div><h3>Opiniones de clientes</h3><p>Agrega solo testimonios reales con autorización para publicarlos.</p></div><button type="button" className="secondary-action" disabled={testimonials.length >= 8} onClick={() => onChange("testimonials", [...testimonials, { quote: "", name: "", detail: "" }])}>Agregar opinión</button></div>{testimonials.length ? <div className="trust-editor-list">{testimonials.map((item, index) => <article className="trust-editor-item" key={`testimonial-${index}`}><div className="trust-editor-item-head"><strong>Opinión {index + 1}</strong><button type="button" className="text-button danger-text" onClick={() => removeItem("testimonials", index)}>Quitar</button></div><label>Opinión<textarea value={item.quote || ""} maxLength="500" rows="3" onChange={(event) => updateItem("testimonials", index, "quote", event.target.value)} placeholder="La experiencia fue clara y sin presión." /></label><div className="form-grid"><label>Nombre<input value={item.name || ""} maxLength="120" onChange={(event) => updateItem("testimonials", index, "name", event.target.value)} placeholder="María R." /></label><label>Contexto<input value={item.detail || ""} maxLength="180" onChange={(event) => updateItem("testimonials", index, "detail", event.target.value)} placeholder="Compró un SUV · Santo Domingo" /></label></div></article>)}</div> : <p className="trust-editor-empty">Todavía no hay opiniones publicadas.</p>}</div></div>;
}
