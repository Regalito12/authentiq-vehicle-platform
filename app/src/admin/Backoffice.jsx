...
function AdminBrandLogo({ brand, size = "normal" }) { const slug = adminBrandLogoSlugs[brand]; const src = slug?.startsWith("http") ? slug : slug ? `https://cdn.simpleicons.org/${slug}` : ""; return <span className={`admin-brand-logo ${size}`}><img src={src} alt={`${brand} logo`} onError={(event) => { event.currentTarget.style.display = "none"; event.currentTarget.parentElement?.classList.add("has-fallback"); }} /><b aria-hidden="true">{brand?.slice(0, 2).toUpperCase()}</b></span>; }
