...
function QuotesModule({ quotes, leads, vehicles, loading, onRefresh, onCreate, onStatusChange, onShare }) {
...
function SettingsModule({ form, organization, onChange, onOrganizationChange, onSave, onOrganizationSave, loading, message, organizationMessage }) {
  return <section className="records-content"><div className="panel-heading"><div><span className="eyebrow">MARCA · OPERACIÓN</span><h2>Configuración.</h2></div></div><div className="settings-form admin-form"><form onSubmit={onOrganizationSave} className="settings-section organization-profile"><span className="eyebrow">PERFIL DEL CONCESIONARIO</span><p className="settings-section-note">Esta identidad será la base para tu futura marca blanca y tu dominio personalizado.</p><div className="form-grid"><label>Nombre del concesionario<input value={organization.name || ""} onChange={(event) => onOrganizationChange("name", event.target.value)} required /></label><label>Slug / identificador<input value={organization.slug || ""} onChange={(event) => onOrganizationChange("slug", event.target.value)} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" required /><small>Ejemplo: concesionario-jordi</small></label><label>Logo del concesionario<input value={organization.logoUrl || ""} onChange={(event) => onOrganizationChange("logoUrl", event.target.value)} placeholder="/uploads/logo.webp" /></label><label>Dominio personalizado<input value={organization.customDomain || ""} onChange={(event) => onOrganizationChange("customDomain", event.target.value)} placeholder="www.concesionario.com" inputMode="url" /><small>Después de guardarlo, apunta este dominio al hosting.</small></label></div>{organizationMessage && <p className="form-message">{organizationMessage}</p>}<button className="primary-action" type="submit">Guardar perfil</button></form><form onSubmit={onSave}>{loading ? <p className="empty-state">Cargando configuración…</p> : <><div className="settings-section"><span className="eyebrow">CONTACTO Y SITIO</span><div className="form-grid"><label>Teléfono<input value={form.phone || ""} onChange={(event) => onChange("phone", event.target.value)} /></label><label>WhatsApp<input value={form.whatsapp || ""} onChange={(event) => onChange("whatsapp", event.target.value)} /></label><label>Correo<input type="email" value={form.email || ""} onChange={(event) => onChange("email", event.target.value)} /></label><label>Moneda<input value={form.currency || "USD"} onChange={(event) => onChange("currency", event.target.value)} maxLength="8" /></label></div><label>Dirección<input value={form.address || ""} onChange={(event) => onChange("address", event.target.value)} /></label><label>Horario visible<input value={form.hours || ""} onChange={(event) => onChange("hours", event.target.value)} placeholder="Lun–Sáb · 9:00–18:00" /></label></div><AppointmentSettingsFields form={form} onChange={onChange} /><div className="settings-section"><span className="eyebrow">CANALES</span><div className="form-grid"><label>Instagram<input value={form.instagramUrl || ""} onChange={(event) => onChange("instagramUrl", event.target.value)} /></label><label>Facebook<input value={form.facebookUrl || ""} onChange={(event) => onChange("facebookUrl", event.target.value)} /></label></div></div>{message && <p className="form-message">{message}</p>}<button className="primary-action" type="submit">Guardar configuración</button></>}</form></div></section>;
}
...
function AppointmentsModule({ appointments, blocks, loading, onRefresh, onStatusChange, onCreateBlock, onDeleteBlock, canManageBlocks }) {
  const [dateFilter, setDateFilter] = useState("");
...
export default function Backoffice({ onBack, onVehiclesChanged }) {
  ...
  const saveOrganization = async (event) => { event.preventDefault(); setOrganizationMessage(""); try { const payload = await request("/api/admin/organization", { method: "PATCH", body: JSON.stringify(organization) }); setOrganization(payload.data); setOrganizationMessage("Perfil guardado correctamente"); } catch (error) { setOrganizationMessage(error.message); } };
