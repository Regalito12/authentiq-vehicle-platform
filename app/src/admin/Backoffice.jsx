...
function InventoryFieldSections({ fields, fieldLabels, numericFields, form, onChange }) {
  const groups = [
    { id: "identity", title: "Identidad y precio", note: "La información principal de la ficha.", fields: ["brand", "category", "model", "variant", "year", "priceUsd"] },
  ...
  const renderField = (field) => <label key={field}>{fieldLabels[field]}<input type={numericFields.includes(field) ? "number" : "text"} value={form[field] ?? ""} onChange={(event) => onChange(field, event.target.value)} required={["brand", "model", "year", "priceUsd"].includes(field)} /></label>;
  return <div className="inventory-field-sections">{groups.map((group, index) => <details className="inventory-field-section" key={group.id} open={index === 0}><summary><span><strong>{group.title}</strong><small>{group.note}</small></span><b>{group.fields.length + (group.id === "availability" ? 2 : 0)} campos</b></summary><div className="form-grid">{group.fields.map(renderField)}{group.id === "availability" && <><label>Estado<select value={form.status} onChange={(event) => onChange("status", event.target.value)}><option value="draft">Borrador</option><option value="pending_review">En revisión</option><option value="published">Publicado</option><option value="reserved">Reservado</option><option value="sold">Vendido</option><option value="inactive">Inactivo</option></select></label><label>Condición<select value={form.condition} onChange={(event) => onChange("condition", event.target.value)}><option value="new">Nuevo</option><option value="used">Usado</option></select></label></>}</div></details>)}</div>;
}
...
function InventoryModule({ vehicles, form, editingId, loading, message, onChange, onSave, onEdit, onCancel, onDeactivate, onDuplicate, onRefresh, onUpload, onPackageUpload, onReview, onStatusChange }) {
  const fields = ["brand", "category", "model", "variant", "year", "priceUsd", "stockNumber", "engine", "power", "transmission", "drive", "fuelType", "exteriorColor", "interiorColor", "doors", "seats", "mileageKm", "location", "warranty", "features", "description", "seoTitle", "seoDescription", "stock", "maxDiscountPercent"];
