...
function Breadcrumbs({ vehicle, onBack = () => {} }) {
  const returnToCatalog = (event) => {
    event.preventDefault();
    onBack();
  };
  return <nav className="breadcrumbs" aria-label="Ruta de navegación"><a href="/" onClick={returnToCatalog}>Inventario</a><span aria-hidden="true">/</span><a href="/" onClick={returnToCatalog}>{vehicle.brand}</a><span aria-hidden="true">/</span><span aria-current="page">{vehicle.model}</span></nav>;
}
...
function VehicleDetail({ vehicle, vehicles = [], onBack, isFavorite = false, onToggleFavorite = () => {}, customerToken = "", compareVehicles = [], favoriteIds = [], onOpenVehicle = () => {}, onToggleCompare = () => {}, whatsapp = "" }) {
  ...
  return (
    ...
    >
      ...
      <Breadcrumbs vehicle={vehicle} onBack={onBack} />
...
function ShowroomNav({ theme, setTheme, customer, onAccount, onBackoffice, onRegisterDealer, businessName = "ZEVROA", logoUrl = "", demoMode = false }) {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event) => {
      if (!event.target.closest?.(".showroom-nav")) setMenuOpen(false);
    };
  ...
  return (
    <><a className="skip-link" href="#top">Saltar al contenido</a><main id="top">
      ...
      <section className="catalog" id="catalog" aria-busy={loading}>
        <div className="section-head"><div><span className="eyebrow">INVENTARIO · {vehicles.length.toString().padStart(2, "0")} MODELOS</span><h2>Vehículos disponibles.</h2></div><p>Compara fotos, detalles y precio en un mismo lugar.</p></div>
         ...
         <div className={`filters ${filtersOpen ? "filters-expanded" : ""}`}>
          ...
          <select value={sort} onChange={(event) => setSort(event.target.value)} aria-label="Ordenar vehículos"><option value="newest">Más recientes</option><option value="price-low">Precio menor</option><option value="price-high">Precio mayor</option><option value="year">Año más reciente</option><option value="mileage">Menor kilometraje</option></select>
           {(search || brand !== "all" || category !== "all" || condition !== "all" || fuelType !== "all" || transmission !== "all" || minPrice || maxPrice || maxMonthlyPayment || minYear || favoritesOnly || sort !== "newest") && <button className="clear-filters" type="button" onClick={clearFilters}>Limpiar</button>}
         </div>
        ...
        {!loading && !error && (filteredVehicles.length ? <div className={`vehicle-grid ${catalogView === "list" ? "is-list-view" : ""}`}><AnimatePresence mode="popLayout" initial={false}>{filteredVehicles.map((vehicle, index) => <VehicleCard key={vehicle.id} vehicle={vehicle} isCompared={compareVehicles.some((item) => item.id === vehicle.id)} isFavorite={favoriteIds.includes(vehicle.id)} onToggleFavorite={toggleFavorite} onToggleCompare={toggleCompare} onOpen={(item) => navigate(vehiclePath(item))} imageLoading={index < 3 ? "eager" : "lazy"} />)}</AnimatePresence></div> : <div className="catalog-empty"><span className="eyebrow">{favoritesOnly ? "MIS FAVORITOS" : maxMonthlyPayment ? "BUSCAR POR CUOTA" : "BÚSQUEDA PERSONALIZADA"}</span><h3>{favoritesOnly ? "Todavía no tienes favoritos." : maxMonthlyPayment ? "No encontramos vehículos en esa cuota." : "No encontramos vehículos con esos criterios."}</h3><p>{favoritesOnly ? "Pulsa el corazón en cualquier ficha para guardarla y volver después." : maxMonthlyPayment ? "Prueba con una cuota mayor o ajusta la inicial y el plazo." : "Prueba limpiando la búsqueda o, si buscas algo específico, déjanos encontrarlo por ti."}</p><div className="catalog-empty-actions"><button type="button" className="secondary-action" onClick={clearFilters}>Limpiar filtros</button>{!favoritesOnly && <button type="button" className="text-button" onClick={() => setBuyerRequestKind("search-alert")}>Pedir ayuda al equipo →</button>}</div></div>)}
