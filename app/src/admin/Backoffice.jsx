...
function IntegrationsModule({ integrations, billing, health, drafts, vehicles, organization, settings, loading, onRefresh, onCreateDraft, onExportCalendar, onConnectGoogleCalendar }) {
  ...
  const statusLabel = (item) => ({ ready: "LISTO", oauth_ready: "OAUTH LISTO", connected: "CONECTADO", local_export_ready: "LISTO EN LOCAL", drafts_ready: "BORRADORES LISTOS", checkout_ready: "CHECKOUT LISTO", verified: "VERIFICADO", dns_pending: "DNS PENDIENTE", not_configured: "SIN CONFIGURAR", local_demo: "DEMO LOCAL" }[item?.status] || "PENDIENTE");
...
export default function Backoffice({ onBack, onVehiclesChanged, initialMode = "login", impersonation = null }) {
  ...
  const connectGoogleCalendar = async () => { try { const payload = await request("/api/admin/integrations/google-calendar/connect"); if (!payload?.data?.authorizationUrl) throw new Error("No se pudo preparar la autorización de Google"); window.location.assign(payload.data.authorizationUrl); } catch (error) { setMessage(error.message); } };
