...
function InventoryImportModal({ open, onClose }) {
  ...
  const submit = async () => {
    ...
    for (const row of validRows) {
      ...
      const body = { ...emptyVehicle, ...row, brandLogoUrl: adminBrandLogoSlugs[row.brand] || "", year: Number(row.year), priceUsd: Number(row.priceUsd), mileageKm: Number(row.mileageKm || 0), stock: Number(row.stock || 1), status: "draft", condition: row.condition || "used", features: String(row.features || "").split(",").map((item) => item.trim()).filter(Boolean), images, imageAltTexts: images.map(() => `${row.brand} ${row.model}`), media: [] };
...
function AdminNav({ activeModule, onChange, onBack, onLogout, role, unreadNotifications, notifications, onReadNotifications, onPreview, theme, onToggleTheme }) {
  ...
  return (
    <>
      ...
      <nav className="admin-nav" aria-label="Módulos administrativos">
        {visibleItems.map(([key, label]) => <button key={key} className={activeModule === key ? "admin-nav-item active" : "admin-nav-item"} onClick={() => onChange(key)}>{label}</button>)}
      </nav>
      ...
      {activeModule === "inventory" && <InventoryImportModal open={importOpen} onClose={() => setImportOpen(false)} />}
    </>
  );
}
...
const adminBrandLogoSlugs = { Acura: "acura", "Alfa Romeo": "alfaromeo", Audi: "https://cdn.freebiesupply.com/logos/large/2x/audi-14-logo-png-transparent.png", Bentley: "bentley", BMW: "https://cdn.freebiesupply.com/logos/large/2x/bmw-logo-png-transparent.png", Buick: "buick", BYD: "byd", Cadillac: "cadillac", Changan: "changan", Chevrolet: "chevrolet", Chrysler: "chrysler", Citroen: "citroen", Daihatsu: "daihatsu", Dodge: "dodge", Ferrari: "ferrari", Fiat: "fiat", Ford: "ford", GMC: "gmc", Honda: "honda", Hyundai: "hyundai", Infiniti: "infiniti", Jaguar: "jaguar", Jeep: "jeep", Kia: "kia", Lamborghini: "lamborghini", "Land Rover": "landrover", Lexus: "lexus", Lotus: "lotus", Maserati: "maserati", Mazda: "mazda", McLaren: "mclaren", MINI: "mini", Mitsubishi: "mitsubishi", Nissan: "nissan", Peugeot: "peugeot", Polestar: "polestar", Porsche: "https://cdn.freebiesupply.com/logos/large/2x/porsche-logo-png-transparent.png", RAM: "ram", Renault: "renault", "Rolls-Royce": "rollsroyce", SEAT: "seat", Skoda: "skoda", Subaru: "subaru", Suzuki: "suzuki", Tesla: "tesla", Toyota: "toyota", Volkswagen: "volkswagen", Volvo: "volvo", "Mercedes-AMG": "https://cdn.freebiesupply.com/logos/large/2x/mercedes-benz-logo-png-transparent.png", "Mercedes-Benz": "https://cdn.freebiesupply.com/logos/large/2x/mercedes-benz-logo-png-transparent.png" };
const adminBrandDirectory = Object.keys(adminBrandLogoSlugs).sort((a, b) => a.localeCompare(b));
function AdminBrandLogo({ brand, size = "normal" }) { const slug = adminBrandLogoSlugs[brand]; const src = slug?.startsWith("http") ? slug : slug ? `https://cdn.simpleicons.org/${slug}` : ""; return <span className={`admin-brand-logo ${size}`}><img src={src} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} /><b>{brand?.slice(0, 2).toUpperCase()}</b></span>; }
...
function BrandPicker({ vehicles, form, onChange }) {
  ...
  const choose = (brand) => { onChange("brand", brand); onChange("brandLogoUrl", adminBrandLogoSlugs[brand]?.startsWith("http") ? adminBrandLogoSlugs[brand] : adminBrandLogoSlugs[brand] ? `https://cdn.simpleicons.org/${adminBrandLogoSlugs[brand]}` : ""); setOpen(false); };
  return <section className="brand-picker"><div className="brand-picker-head"><div><span className="eyebrow">IDENTIDAD DE MARCA</span><h3>{form.brand || "Selecciona una marca"}</h3><p>Solo aparecen marcas con vehículos activos en el inventario.</p></div><button type="button" className="secondary-action" onClick={() => setOpen(true)}>Elegir marca</button></div>{form.brand && <div className="brand-picker-selected"><AdminBrandLogo brand={form.brand} size="wizard" /><div><strong>{form.brand}</strong><small>Logo conectado a esta ficha</small></div></div>}{open && <div className="brand-picker-dialog" role="dialog" aria-modal="true" aria-label="Seleccionar marca"><div className="brand-picker-dialog-head"><h3>Selecciona la marca</h3><button type="button" className="wizard-close" onClick={() => setOpen(false)} aria-label="Cerrar marcas">×</button></div><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar marca..." aria-label="Buscar marca" /><div className="brand-picker-grid">{visible.map((brand) => <button type="button" key={brand} className={form.brand === brand ? "is-selected" : ""} onClick={() => choose(brand)}><AdminBrandLogo brand={brand} size="picker" /><strong>{brand}</strong><small>{vehicles.filter((vehicle) => vehicle.brand === brand).length} modelos</small></button>)}</div>{!visible.length && <p className="empty-state">No hay marcas activas que coincidan.</p>}</div>}</section>;
}
