...
UPDATE vehicles
SET seo_title = CONCAT(brand_name.name, ' ', vehicles.model, ' ', vehicles.year, ' | AUTHENTIQ')
FROM vehicle_brands brand_name
WHERE brand_name.id = vehicles.brand_id AND seo_title IS NULL;

UPDATE vehicles
SET seo_description = LEFT(COALESCE(description, CONCAT('Conoce el ', seo_title, ' en AUTHENTIQ.')), 320)
