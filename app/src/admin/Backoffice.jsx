...
const QuotePrintModal = lazy(() => import("./GraphicsStudio.jsx").then((module) => ({ default: module.QuotePrintModal })));
...
function TaxonomyModule({ taxonomy: externalTaxonomy, loading: externalLoading = false, onRefresh: externalRefresh, onCreate: externalCreate, onUpdate: externalUpdate }) {
  ...
  const submit = async (event) => { event.preventDefault(); if (!form.name.trim() || saving) return; setSaving(true); try { await onCreate(kind, form); setForm({ name: "", logoUrl: "" }); } finally { setSaving(false); } };
    ...
    <>
      <header className="admin-header">
        ...
        <div className="admin-title-row"><h1 className="admin-app-title">{businessName} <span>Panel de control</span></h1><span className="role-chip">{role === "admin" ? "DUEÑO" : role === "content_editor" ? "CONTENIDO" : role === "editor" ? "OPERACIÓN" : "VENTAS"}</span></div>
         <div className="admin-header-actions"><button className="secondary-action command-launch" type="button" onClick={() => setCommandOpen(true)}><MagnifyingGlassIcon size={16} aria-hidden="true" />Acciones <kbd>Ctrl K</kbd></button><div className="notification-wrap"><button className="notification-button" type="button" onClick={() => setShowNotifications((current) => !current)} aria-expanded={showNotifications} aria-controls="admin-notification-popover" aria-label={unreadNotifications > 0 ? `Abrir notificaciones, ${unreadNotifications} sin leer` : "Abrir notificaciones"}><BellIcon size={16} weight="regular" aria-hidden="true" />Notificaciones {unreadNotifications > 0 && <span aria-hidden="true">{unreadNotifications}</span>}</button>{showNotifications && <div className="notification-popover" id="admin-notification-popover"><div className="notification-popover-head"><strong>Actividad reciente</strong>{unreadNotifications > 0 && <button className="text-button" type="button" onClick={onReadNotifications}>Marcar leídas</button>}</div>{notifications?.length ? notifications.slice(0, 8).map((notification) => <article className={notification.readAt ? "notification-item" : "notification-item unread"} key={notification.id}><strong>{notification.title}</strong><span>{notification.body}</span><small>{formatDate(notification.createdAt)}</small></article>) : <p className="empty-state">No hay notificaciones nuevas.</p>}</div>}</div>{["admin", "editor"].includes(role) && <button className="secondary-action onboarding-launch-button" type="button" onClick={role === "editor" ? () => onChange("settings") : onOpenOnboarding}><PaintBrushIcon size={16} weight="regular" aria-hidden="true" />Personalizar showroom</button>}<button className="secondary-action theme-toggle" type="button" onClick={onToggleTheme} aria-label={theme === "dark" ? "Cambiar a modo claro" : "Cambiar a modo oscuro"}>{theme === "dark" ? <SunIcon size={16} weight="regular" aria-hidden="true" /> : <MoonIcon size={16} weight="regular" aria-hidden="true" />}{theme === "dark" ? "Modo claro" : "Modo oscuro"}</button><button className="secondary-action" type="button" onClick={onPreview}><EyeIcon size={16} weight="regular" aria-hidden="true" />Vista previa vehículo</button><button className="secondary-action" type="button" onClick={onOpenPublic}><ArrowUpRightIcon size={16} weight="regular" aria-hidden="true" />Mi showroom</button><button className="secondary-action" type="button" onClick={onBack}><HouseIcon size={16} weight="regular" aria-hidden="true" />Inicio ZEVROA</button><button className="secondary-action" type="button" onClick={onLogout}>Cerrar sesión</button></div>
      </header>
...
function VehicleWizard({ vehicles, taxonomy, form, editingId, message, onChange, onSave, onCancel, onUpload, onPackageUpload, onGenerate3d, onRefresh3d, onPreview, saving = false }) {
...
function InventoryTableModule({ vehicles, taxonomy, form, editingId, loading, message, onChange, onSave, onEdit, onCancel, onDeactivate, onDuplicate, onRefresh, onUpload, onPackageUpload, onReview, onStatusChange, onPreview, onOpenSticker, onOpenSocial, onBulk, onExport }) {
  ...
  const submitSave = async (event) => { if (saving) return; setSaving(true); try { await onSave(event); } finally { setSaving(false); } };
  const saveWizard = async (event) => { if (form.status === "published" && !(await confirm({ title: "Publicar vehículo", message: "Quedará visible en el catálogo y podrá compartirse públicamente.", confirmLabel: "Publicar vehículo" }))) return; await submitSave(event); setWizardOpen(false); };
  if (wizardOpen) return <VehicleWizard vehicles={vehicles} taxonomy={taxonomy} form={form} editingId={editingId} message={message} onChange={onChange} onSave={saveWizard} onCancel={closeWizard} onUpload={onUpload} onPackageUpload={onPackageUpload} onPreview={onPreview} saving={saving} />;
...
function InventoryModule(props) {
  ...
  const closeWizard = () => { setWizardOpen(false); onCancel?.(); };
  const submitSave = async (event) => { if (saving) return; setSaving(true); try { await onSave(event); } finally { setSaving(false); } };
  if (wizardOpen) return <VehicleWizard vehicles={vehicles} taxonomy={taxonomy} form={form} editingId={editingId} message={message} onChange={onChange} onSave={submitSave} onCancel={closeWizard} onUpload={onUpload} onPackageUpload={onPackageUpload} onPreview={onPreview || openPreview} saving={saving} />;
...
const leadEventLabels = { lead_new: "Lead recibido", lead_contacted: "Cliente contactado", lead_qualified: "En negociación", lead_closed: "Venta ganada", lead_lost: "Oportunidad perdida", appointment_pending: "Cita solicitada", appointment_confirmed: "Cita confirmada", appointment_completed: "Cita completada", appointment_cancelled: "Cita cancelada", quote_draft: "Cotización en borrador", quote_sent: "Cotización enviada", quote_accepted: "Cotización aceptada", quote_expired: "Cotización vencida", quote_cancelled: "Cotización cancelada", offer_pending: "Oferta recibida", offer_accepted: "Oferta aceptada", offer_rejected: "Oferta rechazada", internal_note: "Nota interna", assignment_changed: "Responsable actualizado", lead_received: "Lead histórico" };
...
function LeadContactActions({ lead, onLoadHistory, onAddAppointment, onCreateQuote }) {
  ...
  const loadHistory = async () => { setHistoryLoading(true); setHistoryError(""); try { setEvents(await onLoadHistory(lead.id)); } catch (error) { setHistoryError(mensajeDeError(error, "No pudimos cargar el historial.")); } finally { setHistoryLoading(false); } };
  const toggleHistory = async () => { const next = !historyOpen; setHistoryOpen(next); if (next && !events && !historyLoading) await loadHistory(); };
  return <div className="lead-history">{lead.appointmentId && <LeadAppointmentBadge lead={lead} />}<div className="lead-history-actions"><WhatsAppAction lead={lead} /><CopyAction value={lead.phone} label="teléfono" /><CopyAction value={lead.email} label="correo" /><button className="text-button" type="button" onClick={toggleHistory}>{historyOpen ? "Ocultar historial" : "Ver historial"}</button>{lead.vehicleId && !lead.appointmentId && <button className="text-button appointment-inline-action" type="button" onClick={() => onAddAppointment?.(lead)}>Agendar cita</button>}{lead.vehicleId && <button className="text-button quote-inline-action" type="button" onClick={() => onCreateQuote?.(lead)}>Crear cotización</button>}</div>{historyOpen && <div className="lead-events" aria-live="polite">{historyLoading ? <span>Cargando historial…</span> : historyError ? <><span role="alert">{historyError}</span><button className="text-button" type="button" onClick={loadHistory}>Reintentar</button></> : events?.length ? events.map((event) => <div className="lead-event" key={event.id}><strong>{leadEventLabels[event.eventType] || String(event.eventType || "Actividad").replaceAll("_", " ")}</strong><span>{event.note || "Sin detalle"}</span><small>{formatDate(event.createdAt)} · {event.actorName || "Sistema"}</small></div>) : <span>Aún no hay eventos registrados.</span>}</div>}</div>;
}
...
function QuoteShareAction({ quote, onShare }) {
  ...
  const share = async () => {
    setStatus("Generando…");
    try {
      const nextUrl = await onShare(quote.id);
      setUrl(nextUrl);
      try {
        if (!navigator.clipboard?.writeText) throw new Error("Clipboard unavailable");
        ...
        setStatus("Enlace generado. Cópialo desde aquí.");
      }
    } catch (error) {
      setStatus(mensajeDeError(error, "No se pudo compartir"));
    }
  };
  return <div className="quote-share-action"><button className="text-button" type="button" onClick={share} disabled={status === "Generando…"}>{status === "Generando…" ? status : "Compartir URL"}</button>{url && <><CopyAction value={url} label="URL pública" /><small role="status" aria-live="polite">{status}</small></>}</div>;
}

function QuotesModule({ quotes, leads, vehicles, loading, onRefresh, onCreate, onStatusChange, onShare, onExport, initialLead, organization, settings }) {
...
function IntegrationsModule({ integrations = [], billing, health, drafts = [], vehicles = [], organization, settings, loading, onRefresh, onCreateDraft, onExportCalendar, onConnectGoogleCalendar, onDisconnectGoogleCalendar, onStartBillingCheckout }) {
...
function SettingsModule({ form, organization, onboarding, onChange, onOrganizationChange, onSave, onOrganizationSave, onUpload, onNavigate, onOpenPublic, loading, message, organizationMessage }) {
  ...
  const [profileLinkCopyError, setProfileLinkCopyError] = useState(false);
...
function BackofficeWorkspace({ onBack, onVehiclesChanged, initialMode = "login", impersonation = null }) {
    ...
    try {
      ...
      if (integrationParam === "google_calendar") {
        if (statusParam === "connected") {
          setMessage("Google Calendar conectado correctamente. Las citas se sincronizarán con tu cuenta.");
        } else if (statusParam === "error") {
          setMessage(`Error al conectar Google Calendar: ${messageParam || "No se pudo autorizar la cuenta"}`);
        }
  ...
  const loadLeadHistory = async (id) => (await request(`/api/admin/leads/${id}/events`)).data || [];
  ...
  const createQuote = async (values) => { try { await request("/api/admin/quotes", { method: "POST", body: JSON.stringify({ ...values, basePriceUsd: Number(values.basePriceUsd), discountUsd: Number(values.discountUsd || 0) }) }); setQuoteLead(null); setMessage("Cotización guardada correctamente"); await Promise.all([loadQuotes(), loadDashboard(), loadWorkQueue(), loadNotifications()]); } catch (error) { setMessage(mensajeDeError(error)); throw error; } };
  const openQuoteForLead = async (lead) => { if (!(await confirmDiscardAdminDraft())) return; setQuoteLead(lead); setActiveModule("quotes"); };
  const updateQuoteStatus = async (id, status) => { if (status === "cancelled" && !(await confirm({ title: "Cancelar cotización", message: "La propuesta dejará de estar disponible para el cliente.", confirmLabel: "Cancelar cotización", danger: true }))) return; try { await request(`/api/admin/quotes/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }); setMessage("Estado de cotización actualizado"); await Promise.all([loadQuotes(), loadDashboard(), loadWorkQueue(), loadNotifications()]); } catch (error) { setMessage(mensajeDeError(error)); } };
  ...
  const disconnectGoogleCalendar = async () => { if (currentUser?.role !== "admin") { setMessage("Esta acción la realiza el administrador."); return; } if (!(await confirm({ title: "Desconectar Google Calendar", message: "Las citas nuevas ya no se sincronizarán con tu cuenta de Google. Podrás volver a conectarla en cualquier momento.", confirmLabel: "Desconectar", danger: true }))) return; try { await request("/api/admin/integrations/google-calendar", { method: "DELETE" }); setMessage("Google Calendar desconectado correctamente"); await loadIntegrations(); } catch (error) { setMessage(mensajeDeError(error)); } };
  ...
  return (
    <main className="admin-page">
      ...
      <AnimatePresence><AdminToast message={message} action={toastAction} onDismiss={() => { setMessage(""); setToastAction(null); }} /></AnimatePresence>
    </main>
  );
}
