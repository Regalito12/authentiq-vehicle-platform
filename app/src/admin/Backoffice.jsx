...
function DashboardView({ data, vehicles = [], leads, offers, appointments, loading, error = "", onRetry, onNavigate, onOpenLead, onboarding, onOpenOnboarding, onOpenPublic, organization, settings, currentUser, workQueue = [], workQueueLoading = false, workQueueError = "", onRefreshWorkQueue }) {
  ...
  return (
    <section className="dashboard-content">
      ...
      <WorkCenter items={workQueue} loading={workQueueLoading} error={workQueueError} currentUser={currentUser} onRefresh={onRefreshWorkQueue} onNavigate={onNavigate} />
...
function UsersModule({ users, form, onChange, onSave, onUpdate, onResetPassword, onDelete, onInvite, onMfaSetup, loading, message }) {
  ...
  const submitInvite = async (event) => { event.preventDefault(); setInviteLoading(true); setInviteState(""); try { await onInvite?.(invite); setInvite({ fullName: "", email: "", role: "seller" }); setInviteState("Invitación enviada. El enlace vence en 72 horas."); } catch (error) { setInviteState(error.message); } finally { setInviteLoading(false); } };
  const startMfa = async () => { setInviteState(""); try { setMfaState(await onMfaSetup?.()); } catch (error) { setInviteState(error.message); } };
  const verifyMfa = async (event) => { event.preventDefault(); try { const formData = new FormData(event.target); const result = await onMfaSetup?.({ setupToken: mfaState.setupToken, code: formData.get("code") }); setMfaState({ recoveryCodes: result?.recoveryCodes || [] }); } catch (error) { setInviteState(error.message); } };
  return (
...
function BackofficeWorkspace({ onBack, onVehiclesChanged, initialMode = "login", impersonation = null }) {
  ...
  const loadWorkQueue = async () => { setWorkQueueLoading(true); setWorkQueueError(""); try { setWorkQueue((await request("/api/admin/work-queue")).data || []); } catch (error) { setWorkQueueError(error.message); } finally { setWorkQueueLoading(false); } };
  ...
  const loadAnalytics = async () => { try { const [events, funnel] = await Promise.all([request("/api/admin/analytics?days=30"), request("/api/admin/analytics/funnel?days=30")]); setAnalytics(events.data || []); setFunnelData(funnel.data || null); } catch (error) { setMessage(error.message); } };
  ...
  const submitMfaChallenge = async (event) => { event.preventDefault(); setLoginError(""); try { const response = await fetch(`${apiUrl}/api/auth/mfa/challenge`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ challengeToken: mfaChallenge, code: mfaCode }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "No se pudo validar el código"); setCurrentUser(payload.user); setToken(payload.token || ""); setMfaChallenge(""); setMfaCode(""); setSessionResolved(true); } catch (error) { setLoginError(error.message); } };
